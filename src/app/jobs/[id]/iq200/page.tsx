...
export default function IQ200JobPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    ]).then(([jobContext, sessionResult, historyResult]) => {
      if (!active) return;
      setContext(jobContext); setSessions(sessionResult.sessions); setHistory(historyResult.results);
  ...
  async function startSession(event: React.FormEvent) {
    ...
    try {
      ...
      setSessions((current) => [result.session, ...current]); setQuestion("");
  ...
  return <main className="min-h-screen bg-[#f5f7fb] p-3 sm:p-6"><div className="mx-auto max-w-6xl space-y-5">
    <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><Link href={`/jobs/${context.job.id}`} className="inline-flex items-center gap-2 text-sm font-bold text-blue-700"><ChevronLeft size={17}/>Back to job</Link><div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-3"><span className="rounded-2xl bg-blue-600 p-3 text-white"><Bot size={28}/></span><div><p className="text-xs font-black uppercase tracking-[0.2em] text-blue-600">Technician Assist</p><h1 className="text-3xl font-black text-slate-950">ASK IQ200</h1></div></div><p className="mt-4 text-sm text-slate-500">Secure job context and related repair history. AI diagnosis is not enabled in Phase 1 or Phase 2.</p></div><span className="inline-flex w-fit items-center gap-2 rounded-full bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-800"><ShieldCheck size={16}/>Company-scoped</span></div></header>
