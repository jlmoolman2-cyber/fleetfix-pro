...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeUsers = onSnapshot(collection(clientDb, "companies", COMPANY_ID, "users"), (snapshot) => setUserNames(Object.fromEntries(snapshot.docs.map((entry) => { const data = entry.data(); return [entry.id, data.name || `${data.firstName || ""} ${data.lastName || ""}`.trim() || data.email || "JobTorq User"]; }))));
        ...
        <table className="w-full min-w-[1450px] table-fixed text-[13px] leading-5 [font-family:Arial,Helvetica,sans-serif]">
          ...
          <tbody>
            {visible.map((item) => <Fragment key={item.id}><tr className={`border-b font-normal ${item.finalized ? "bg-gray-50 text-gray-500" : "bg-white text-gray-900 hover:bg-blue-50/40"}`}>
              ...
              <td className="truncate px-3 py-2" title={item.recipientName || item.assignedUserName || userNames[item.recipientId || item.assignedUserId]}>{item.recipientName || item.assignedUserName || userNames[item.recipientId || item.assignedUserId] || (!item.recipientId && !item.assignedUserId ? "All users" : "JobTorq User")}</td>
