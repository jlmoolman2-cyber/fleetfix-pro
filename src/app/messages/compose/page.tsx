...
function ComposeMessage() {
  ...
  return <main className="min-h-screen bg-[#f4f7fb] p-4 md:p-6"><section className="mx-auto max-w-[1500px] overflow-hidden rounded-2xl border bg-white shadow-sm"><header className="flex items-center justify-between border-b px-5 py-4"><div><h1 className="text-xl font-black">Compose Message</h1><p className="text-sm text-gray-500">{documentId ? `${module} communication` : "New JobTorq communication"}</p></div><button disabled={sending} onClick={() => void send()} className="rounded-lg bg-blue-600 px-5 py-2.5 font-black text-white disabled:opacity-50">✈ {sending ? "Sending…" : "Send Message"}</button></header>
    <div className="space-y-5 p-5"><div className="flex flex-wrap items-center gap-5"><span className="rounded-lg bg-blue-50 px-4 py-2 text-sm font-black text-blue-700">Email</span><label className="ml-auto text-sm font-bold">Module <select disabled={Boolean(documentId)} value={module} onChange={(event) => { setModule(event.target.value); setTemplateId(""); }} className="ml-2 rounded-lg border px-3 py-2 disabled:bg-gray-100">{MESSAGE_TEMPLATE_MODULES.map((item) => <option key={item}>{item}</option>)}</select></label></div>
      <div className="relative"><p className="mb-2 text-xs font-black uppercase text-gray-500">Select recipients</p><button type="button" onClick={() => setShowRecipients((current) => !current)} className="min-h-12 w-full rounded-xl border px-4 text-left">{recipientIds.length ? `${recipientIds.length} recipient${recipientIds.length === 1 ? "" : "s"} selected` : initialModule === "Purchase Order" ? "Select this supplier's contacts or JobTorq users" : ["JobCard", "Quote", "Invoice"].includes(initialModule) ? "Select this customer's contacts or JobTorq users" : "Select recipients"}</button>{showRecipients && <div className="absolute z-30 mt-2 max-h-80 w-full overflow-hidden rounded-xl border bg-white shadow-xl"><input value={recipientSearch} onChange={(event) => setRecipientSearch(event.target.value)} placeholder="Search available recipients..." className="h-12 w-full border-b px-4" /><div className="max-h-64 overflow-y-auto p-2">{filteredRecipients.map((item) => <label key={item.id} className="flex cursor-pointer items-center gap-3 rounded-lg p-3 hover:bg-blue-50"><input type="checkbox" checked={recipientIds.includes(item.id)} onChange={(event) => setRecipientIds((current) => event.target.checked ? [...current, item.id] : current.filter((id) => id !== item.id))} /><span><b>{recipientName(item)}</b><small className="ml-2 text-gray-500">{item.type} · {item.email || item.email1 || item.phone || item.mobile || "No contact details"}</small></span></label>)}</div><button type="button" onClick={() => setShowRecipients(false)} className="w-full border-t p-3 font-bold text-blue-700">Done</button></div>}</div>
