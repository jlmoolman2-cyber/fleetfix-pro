...
export async function reserveHostedRequest(input:{companyId:string;userId:string;jobId:string;sessionId:string;question:string;limits:{perUser:number;perCompany:number;perSession:number;companyPeriodRequests:number;periodSeconds:number}},now=Date.now(),maxAttempts=2,commissioning?:{phase:"PHASE_7";maxRequests:1;ledger?:string;idempotencyNamespace?:string}){
 validateHostedControlIds(input.companyId,input.userId,input.jobId,input.sessionId);const idempotencyKey=hostedIdempotencyKey(input.companyId,input.userId,input.jobId,input.sessionId,input.question,commissioning?.idempotencyNamespace),window=Math.floor(now/(input.limits.periodSeconds*1000)),base=`companies/${input.companyId}/iq200_hosted_controls`,dedupe=adminDb.doc(`${base}/idempotency_${idempotencyKey}`),requestRefFor=(requestId:string)=>adminDb.doc(`${base}/request_${requestId}`),refs=[[adminDb.doc(`${base}/user_${input.userId}_${window}`),input.limits.perUser],[adminDb.doc(`${base}/company_${window}`),Math.min(input.limits.perCompany,input.limits.companyPeriodRequests)],[adminDb.doc(`${base}/session_${input.sessionId}_${window}`),input.limits.perSession]] as const;
 return adminDb.runTransaction(async transaction=>{
  const existing=await transaction.get(dedupe);
  if(existing.exists){
   const requestId=typeof existing.data()?.requestId==="string"?existing.data()?.requestId:null,attempt=existing.data()?.attempt;
   if(!requestId||!Number.isSafeInteger(attempt)||Number(attempt)<1)throw new Error("INVALID_IDEMPOTENCY_STATE");
   const requestSnapshot=await transaction.get(requestRefFor(requestId)),status=requestSnapshot.data()?.status;
   if(status==="SUCCEEDED")return{duplicate:true,inProgress:false,retryExhausted:false,retry:false,requestId,requestRef:requestRefFor(requestId)};
   if(status==="RESERVED"||status===undefined)return{duplicate:true,inProgress:true,retryExhausted:false,retry:false,requestId,requestRef:requestRefFor(requestId)};
   if(status==="FAILED"){
    if(Number(attempt)>=maxAttempts)return{duplicate:true,inProgress:false,retryExhausted:true,retry:false,requestId,requestRef:requestRefFor(requestId)};
    transaction.update(dedupe,{attempt:attempt+1,updatedAt:FieldValue.serverTimestamp()});return{duplicate:false,inProgress:false,retryExhausted:false,retry:true,requestId,requestRef:requestRefFor(requestId)};
   }
   throw new Error("INVALID_IDEMPOTENCY_STATE");
  }
  const snapshots=await Promise.all(refs.map(([ref])=>transaction.get(ref))),counts=snapshots.map((snapshot,index)=>{try{return nextHostedRateCount(snapshot.exists?snapshot.data()?.count:undefined,refs[index][1])}catch(error){if(error instanceof Error&&error.message==="RATE_LIMITED"&&index===1)throw new Error("COMPANY_LIMIT");throw error}}),commissioningRef=commissioning?adminDb.doc(`iq200_hosted_commissioning/${commissioning.ledger||"phase7"}`):null,commissioningSnapshot=commissioningRef?await transaction.get(commissioningRef):null,commissioningCount=commissioningSnapshot?.exists?commissioningSnapshot.data()?.count:0;
