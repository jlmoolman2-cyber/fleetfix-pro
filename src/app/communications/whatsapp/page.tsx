...
type Capabilities = { view: boolean; viewMedia: boolean; manage: boolean; assign: boolean; close: boolean; diagnostics: boolean; manualOutbound: boolean; automationOutbound: boolean; templateOutbound: boolean };
type MetaDiagnostics = { authenticationPassed: boolean; graphApiConnectivityPassed: boolean; phoneNumberIdMatches: boolean; wabaIdMatches: boolean; displayPhoneNumberMatches: boolean };
type Conversation = {
...
export default function WhatsAppInboxPage() {
  ...
  const [capabilities, setCapabilities] = useState<Capabilities>({ view: false, viewMedia: false, manage: false, assign: false, close: false, diagnostics: false, manualOutbound: false, automationOutbound: false, templateOutbound: false });
  ...
  const runMetaDiagnostics = async () => {
    if (!capabilities.diagnostics || diagnosticsLoading) return;
    try {
      setDiagnosticsLoading(true); setDiagnostics(null); setDiagnosticsError("");
      const result = await whatsappApi<{ meta: MetaDiagnostics | null }>("/api/whatsapp/diagnostics?liveProbe=true");
      if (!result.meta) throw new Error("Meta diagnostics returned no result.");
      setDiagnostics(result.meta);
    } catch (reason) {
      setDiagnosticsError(reason instanceof Error ? reason.message : "Read-only Meta diagnostics failed.");
  ...
  return <main className="flex h-full min-h-0 flex-col bg-[#f5f7fb] p-3 md:p-5">
    ...
    {capabilities.diagnostics && <details data-testid="whatsapp-meta-diagnostics" className="mb-3 rounded-xl border border-blue-200 bg-blue-50 p-3"><summary className="cursor-pointer text-xs font-black text-blue-800">Read-only Meta Diagnostics</summary><p className="mt-2 text-xs text-slate-600">Runs authenticated Meta configuration checks only. It cannot send WhatsApp messages.</p><button type="button" onClick={() => void runMetaDiagnostics()} disabled={diagnosticsLoading} className="mt-2 rounded-lg bg-blue-700 px-3 py-2 text-xs font-black text-white disabled:opacity-50">{diagnosticsLoading ? "Running read-only checks…" : "Run Read-only Meta Diagnostics"}</button>{diagnostics && <dl className="mt-3 grid gap-1 text-xs">{[["Meta authentication", diagnostics.authenticationPassed], ["Graph API connectivity", diagnostics.graphApiConnectivityPassed], ["Phone Number ID match", diagnostics.phoneNumberIdMatches], ["WABA ID match", diagnostics.wabaIdMatches], ["Display/test number match", diagnostics.displayPhoneNumberMatches]].map(([label, passed]) => <div key={String(label)} className="flex justify-between gap-3"><dt>{String(label)}</dt><dd className={passed ? "font-black text-emerald-700" : "font-black text-red-700"}>{passed ? "PASS" : "FAIL"}</dd></div>)}</dl>}{diagnosticsError && <p role="alert" className="mt-3 text-xs font-bold text-red-700">{diagnosticsError}</p>}</details>}
