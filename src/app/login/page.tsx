...
export default function LoginPage() {
  ...
  return <main className="min-h-screen w-full bg-[#071426] text-slate-900 lg:grid lg:grid-cols-[1.08fr_0.92fr]">
    <section className="relative hidden min-h-screen overflow-hidden bg-gradient-to-br from-[#0a2a51] via-[#0b3768] to-[#071426] p-12 text-white lg:flex lg:flex-col lg:justify-between xl:p-16">
      ...
      <p className="relative text-xs font-semibold text-blue-200/60">JobTorq · Built for modern service teams</p>
    </section>
    ...
    <section className="flex min-h-screen items-center justify-center bg-[#f4f7fb] px-5 py-10 sm:px-10"><div className="w-full max-w-md">
      ...
      <div className="mb-8"><div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-100 text-blue-700"><ShieldCheck size={24} /></div><h2 className="text-4xl font-black tracking-tight text-slate-950">Welcome back</h2><p className="mt-3 text-slate-500">Sign in to manage your JobTorq workspace.</p></div>
      <form onSubmit={submit} className="space-y-5">
        ...
        <button disabled={loading} type="submit" className="flex h-14 w-full items-center justify-center gap-3 rounded-2xl bg-[#0b3768] font-black text-white shadow-lg shadow-blue-900/15 transition hover:bg-[#0d477f] disabled:opacity-60">{loading ? "Signing in…" : "Sign in to JobTorq"}{!loading && <ArrowRight size={19} />}</button>
      </form>
...
function Brand({ dark = false }: { dark?: boolean }) { return <Image src={dark ? "/brand/jobtorq-logo-light.png" : "/brand/jobtorq-logo-dark.png"} alt="JobTorq — Workshop & Fleet Operations Platform" width={320} height={96} className="h-auto w-full max-w-[320px]" priority />; }
