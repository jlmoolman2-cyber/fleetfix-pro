...
export type OpenAIResponseEnvelope = { status?: string; output_text?: string; output?: Array<{ type?: string; content?: Array<{type?:string}> }>; usage?: { input_tokens?: unknown; output_tokens?: unknown } };
...
export function parseOpenAIResponseEnvelope(envelope: OpenAIResponseEnvelope): { response: unknown; usage: { inputUnits: number | null; outputUnits: number | null } } {
    const output=envelope?.output,supportedOutput=Array.isArray(output)&&output.every(item=>item?.type==="message"||item?.type==="reasoning"),hasMessage=Array.isArray(output)&&output.some(item=>item?.type==="message"),hasRefusal=Array.isArray(output)&&output.some(item=>item?.type==="message"&&item.content?.some(part=>part?.type==="refusal"));
    if (!envelope || typeof envelope !== "object" || envelope.status !== "completed" || !Array.isArray(output) || output.length < 1 || !supportedOutput || !hasMessage || hasRefusal || typeof envelope.output_text !== "string" || !envelope.output_text.trim() || envelope.output_text.length > OUTPUT_TEXT_MAX) throw new HostedProviderError("INVALID_PROVIDER_RESPONSE");
