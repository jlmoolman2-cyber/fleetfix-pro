...
export default function WhatsAppInboxPage() {
  ...
  const openConversation = (id: string) => { setSelectedId(id); setJobSearch(""); void loadConversation(id); };
      ...
      <div className={`${selectedId ? "flex" : "hidden md:flex"} min-w-0 flex-1 flex-col bg-[#efeae2]`}>
        {!selectedId ? <div className="flex h-full flex-col items-center justify-center text-center"><MessageCircle className="h-14 w-14 text-emerald-500" /><h2 className="mt-4 text-xl font-black text-slate-700">FleetFix WhatsApp</h2><p className="mt-2 max-w-sm text-sm text-slate-500">Select a conversation to review its messages and FleetFix association.</p></div> : loadingConversation || !selected ? <div className="flex h-full items-center justify-center"><Loader2 className="animate-spin text-emerald-600" /></div> : <>
          <header className="border-b bg-white px-4 py-3 shadow-sm">
            <div className="flex items-center gap-3"><button onClick={() => { setSelectedId(null); setSelected(null); }} className="md:hidden" aria-label="Back to conversations"><ArrowLeft /></button><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-emerald-700"><UserRound size={19} /></div><div className="min-w-0 flex-1"><div className="truncate font-black">{selected.contactName || selected.customerName || "Unknown contact"}</div><div className="truncate text-xs text-slate-500">{selected.customerName || "Unknown customer"} · {selected.phoneNumber}</div></div><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${selected.status === "closed" ? "bg-slate-200 text-slate-600" : "bg-emerald-100 text-emerald-700"}`}>{selected.status}</span></div>
            <div className="mt-3 flex flex-wrap items-center gap-2 border-t pt-3 text-xs">
              {selected.customerId ? <Link href={selected.contactId ? `/customers/${selected.customerId}/contacts/${selected.contactId}` : `/customers/${selected.customerId}`} className="font-black text-blue-700 hover:underline">{selected.contactName || selected.customerName}</Link> : <span className="rounded bg-amber-100 px-2 py-1 font-black text-amber-800">Unknown contact</span>}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {!selected.customerId && <button disabled title="Customer/contact association will be enabled in a controlled later phase" className="h-9 rounded-lg border bg-slate-100 px-3 text-xs font-bold text-slate-400">Associate customer/contact</button>}
            </div>
