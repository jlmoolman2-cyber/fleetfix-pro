...
export async function runHostedReasoning(context:ServerUserContext,jobId:string,sessionId:string,question:string,currentData:DocumentData,sessionRef:FirebaseFirestore.DocumentReference,overrides:{config?:HostedReasoningConfig;provider?:IQ200ReasoningProvider;transport?:HostedReasoningTransport;timeoutMs?:number;gate?:(config:HostedReasoningConfig)=>boolean}={}):Promise<HostedExecutionOutcome>{
 ...
 const controls:HostedExecutionControls={
  ...
  persistFailure:async({scope,requestId,errorClass,validationReason,latencyMs})=>{const interaction=sessionRef.collection("interactions").doc(),requestRef=adminDb.doc(`companies/${scope.companyId}/iq200_hosted_controls/request_${requestId}`),commissioningRef=adminDb.doc(PHASE7_RECOVERY_LEDGER_PATH),batch=adminDb.batch(),safeValidationMetadata=validationReason?{validationReason}:{};batch.create(interaction,{companyId:scope.companyId,jobId:scope.jobId,sessionId:scope.sessionId,requestedBy:scope.userId,featureState:"HOSTED",provider:"openai",requestId,success:false,errorClass,...safeValidationMetadata,latencyMs,createdAt:FieldValue.serverTimestamp()});batch.update(requestRef,{status:"FAILED",errorClass,...safeValidationMetadata,updatedAt:FieldValue.serverTimestamp()});batch.update(commissioningRef,{status:"FAILED",errorClass,...safeValidationMetadata,latencyMs,updatedAt:FieldValue.serverTimestamp()});await batch.commit()},
