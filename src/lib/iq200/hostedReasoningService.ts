...
import { hostedExecutionAllowed, type HostedReasoningConfig } from "./hostedConfig";
...
export async function runHostedReasoning(context:ServerUserContext,jobId:string,sessionId:string,question:string,currentData:DocumentData,sessionRef:FirebaseFirestore.DocumentReference,overrides:{config?:HostedReasoningConfig;provider?:IQ200ReasoningProvider;transport?:HostedReasoningTransport;timeoutMs?:number;gate?:(config:HostedReasoningConfig)=>boolean}={}):Promise<HostedExecutionOutcome>{
 ...
 const controls:HostedExecutionControls={
  ...
  reserve:async inner=>hostedReservation(await reserveHostedRequest({companyId:inner.companyId,userId:inner.userId,jobId:inner.jobId,sessionId:inner.sessionId,question:inner.question,limits:config.limits!})),
  ...
  persistSuccess:async({scope,requestId,response,provider,model,usage,latencyMs})=>{const interaction=sessionRef.collection("interactions").doc(),requestRef=adminDb.doc(`companies/${scope.companyId}/iq200_hosted_controls/request_${requestId}`),batch=adminDb.batch();batch.create(interaction,{companyId:scope.companyId,jobId:scope.jobId,sessionId:scope.sessionId,question:scope.question,response,evidenceReferences:response.evidenceUsed,requestedBy:scope.userId,featureState:"HOSTED",provider,model,usage,requestId,success:true,latencyMs,createdAt:FieldValue.serverTimestamp()});batch.update(sessionRef,{state:"REASONING_AVAILABLE",responseStatus:"STRUCTURED_RESPONSE",updatedAt:FieldValue.serverTimestamp()});batch.update(requestRef,{status:"SUCCEEDED",interactionId:interaction.id,updatedAt:FieldValue.serverTimestamp()});await batch.commit();return interaction.id},
  persistFailure:async({scope,requestId,errorClass,latencyMs})=>{const interaction=sessionRef.collection("interactions").doc(),requestRef=adminDb.doc(`companies/${scope.companyId}/iq200_hosted_controls/request_${requestId}`),batch=adminDb.batch();batch.create(interaction,{companyId:scope.companyId,jobId:scope.jobId,sessionId:scope.sessionId,question:scope.question,requestedBy:scope.userId,featureState:"HOSTED",provider:"openai",requestId,success:false,errorClass,latencyMs,createdAt:FieldValue.serverTimestamp()});batch.update(requestRef,{status:"FAILED",errorClass,updatedAt:FieldValue.serverTimestamp()});await batch.commit()},
